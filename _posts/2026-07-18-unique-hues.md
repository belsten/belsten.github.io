...
excerpt: "In this blog post, I explain our recent paper on color vision."
date: 2026-07-28
categories:
  - blog
tags:
...
<p style="margin: 0 0 0.15em;">Alexander Belsten, E. Paxon Frady, and Bruno A. Olshausen</p>
<p style="margin: 0 0 1.5em; color: #666; font-size: 0.95em;">Published July 14th, 2026 in <em>JOSA A</em>. <a href="https://doi.org/10.1364/JOSAA.598897">https://doi.org/10.1364/JOSAA.598897</a></p>

In this blog post, I explain our recent findings on color vision for a general audience. The paper makes two main contributions. First, we show that the distribution of colors in natural images is highly non-random. Second, we show that when a statistical model called sparse coding is fit to this distribution, it converges to a specific set of basis vectors in color space that are also considered ‘unique’ by humans.


I will keep the specific details of color to a minimum, as color can get quite technical. Of course, the details are in the paper. There are also many excellent resources on color vision, including Brian Wandell’s *Foundations of Vision*.


Sparse coding also comes with its own jargon and complexities. Here, I hope to shed some of that, as applying this method to a low-dimensional color space lends itself to an intuitive geometric understanding of what this model is doing.
...
<figure style="text-align: center;">
 ...
 <figcaption><strong>Figure 1</strong>: Hering’s diagram of the <i>Urfarben</i> from <i>Zur Lehre vom Lichtsinne</i> (1878).. The top panel shows how different ratios of the unique hues combine to make the various hues shown on the bottom panel. Mutually exclusive unique hues–red/green and blue/yellow–are opposite to one another.</figcaption>
</figure>
...
The animation in Figure 2 shows the process of inference for three colors using the Mercedes-Benz frame:
...
Each basis vector is rescaled by its corresponding coefficient, and geometrically placing the scaled vectors end to end shows how their sum reaches the target color. Because the basis vectors have unit length, the sum of the coefficients is also the total path length traced by the scaled vectors. In general, more saturated colors will have a longer path length. Sparse inference favors a short path to the target. In this two-dimensional example, at most two basis vectors are necessary to represent any color, and only one is needed when the target lies directly along a basis direction.


With this geometric view, we can see how activating all three vectors would introduce unnecessary backtracking. Since $\mathbf{x}$ is 2-dimensional, the minimum-path representation uses no more than two active coefficients. This is not unique to the Mercedes-Benz frame or even having three basis vectors. Given a basis of size $m>n+1$ that spans an $n$-dimensional space under nonnegative weightings, only $n$ nonzero coefficients are needed to represent a point in that space.


The statistical model we’re describing is called **sparse coding**. Sparse coding assumes that the data can be represented using sparse coefficients on a collection of basis vectors (Olshausen & Field, 1996). The mathematical details of sparse coding and sparse inference are described below. Readers interested primarily in the main results can skip this box.
...
We seek to understand how color is distributed in this dataset. 225 million data points is far too many to put in a scatterplot. Instead, we project the entire three-dimensional distribution of pixels onto the chromatic plane and compute a histogram of the projections. We then summarize this histogram using iso-probability contours, as shown below:
...
The mathematical details of the learning rule are given below. This box can be skipped without disrupting the main results.
...
The basis vectors $\mathbf{a}_i$ are adapted to the structure of the data by stochastic gradient descent on $E$, using the inferred coefficients $s_i$ for each data sample $\mathbf{x}$. This yields the following learning rule:
...
The update rate $\eta$ is set so that the energy decreases continuously until convergence. After each update, the basis vectors are normalized to have unit norm: $\lVert\mathbf{a}_i\rVert_2 =1 \text{ for all } i$.
</div>
